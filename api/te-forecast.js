...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .clash-badge-global.backtoback { background: var(--gold); color: #fff; }
  ...
  .person-item.has-backtoback .person-header { border-left: 3px solid var(--gold); }
  ...
  .clash-status.backtoback { background: var(--gold-soft); color: var(--gold); }
  .person-body {
    max-height: 0; opacity: 0; overflow: hidden; padding: 0 20px;
    transition: max-height 0.28s ease, opacity 0.2s ease, padding 0.28s ease;
  }
  ...
  .backtoback-row { background: var(--gold-soft); }
  .backtoback-row:hover { background: #F2E5C8 !important; }
  .backtoback-flag { display: inline-flex; align-items: center; gap: 4px; font-size: 10px; font-weight: 700; color: var(--gold); font-family: 'IBM Plex Mono', monospace; }
...
<div class="wrap">
  ...
  <footer>
    ...
    Rows with no onsite dates are excluded from the clash check since there's nothing to compare. A flagged pair
    is split into two kinds: a <b>clash</b> (different cities — a genuine scheduling conflict) and a
    <b>back-to-back</b> (same city, just a same-day handoff between events — not a problem, just worth knowing
    about). Within each expanded conference, the team is ordered Reg Lead → Reg Support → IT Lead → IT Support →
    other roles → Zone Leads. The <span class="cross-region-badge" style="margin-left:0;">⇄ REGION</span> badge
    marks someone whose home region (from the "Team View" tab) differs from the conference's region — this is
    only shown for the ~49 people who appear in that tab; others can't be checked against it.
  </footer>
</div>
